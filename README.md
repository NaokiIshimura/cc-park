...
**4 本まとめて半セルぶん**中央へ寄ってから、標準の形へ戻ります。
...
AA は半セルより細かく動かせないため、**開いた形は標準そのもの**にして振り幅を抑えています。
...
`BUSY` のときは 4 本まとめて**半セルぶん**中央へ寄り、標準の形へ戻ります。
...
半セルより細かくは動かせないので、開いた形は標準そのものにして振り幅を抑えています。
足の本数（4 本）と左右対称はどちらのフレームでも変わりません。
標準の形は静止する状態（`IDLE` など）でも使います。
