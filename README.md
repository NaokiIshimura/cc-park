...
| 表示 | 色 | 手の動き | 意味 |
| --- | --- | --- | --- |
| `BLOCKED` | 黄 | 下ろす ⇄ 両手を高く上げる（大きく振って呼ぶ） | 承認・入力を待っている（要対応） |
| `DONE!` | 明るい緑 | 少し上げる ⇄ 高く上げる（上げたまま揺れる） | 作業が完了した直後（既定 10 秒間） |
| `BUSY` | シアン | 左手 → 下ろす → 右手 → 下ろす（交互に振る） | 作業中 |
| `IDLE` | 灰 | 静止 | 入力待ち |
...
手や足に使う文字は、身体と縦横に接する象限を持つものだけに限定しています。
たとえば桁 0 の手に `▘`（左上）を使うと身体と接せず、部品が浮いて見えます。
