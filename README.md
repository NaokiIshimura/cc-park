...
| `BUSY` | シアン | 足 | 8 | 右足 → 左足 の順に左手側へ開いて閉じる | 作業中 |
| `IDLE` | 灰 | - | 1 | 静止 | 入力待ち |
...
- 説明文: `working...`
- 動き: **右足を開く → 標準 → 右足を閉じる → 標準 → 左足を開く → 標準 → 左足を閉じる → 標準**（8 フレーム / 1600ms 周期）。踏み出す向きは常に**左手側**です
...
「開く」は左手寄りの足が隣の桁へ出て `▘`（左上）になり、
「閉じる」は右手寄りの足が寄ってペアが 1 つの `▀`（上半分）に繋がります。
...
「開く」は左手寄りの足が隣の桁へ出て `▘` になり、
「閉じる」は右手寄りの足が寄ってペアが 1 つの `▀` に繋がります。
