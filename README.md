...
キャラクターは Claude Code の起動バナーと同じマークです。身体と足は全状態で共通で、
**1 行目の両端にある「手」の動きと色**で状態を表します。
...
| 表示 | 色 | 手の動き | 意味 |
| --- | --- | --- | --- |
| `BLOCKED` | 黄 | 右手 ⇄ 下ろす（右手を上げ下げして呼ぶ） | 承認・入力を待っている（要対応） |
| `DONE!` | 明るい緑 | 下ろす ⇄ 両手（両手を上げ下げして喜ぶ） | 作業が完了した直後（既定 10 秒間） |
| `BUSY` | シアン | 右手 → 下ろす → 左手 → 下ろす（片手ずつ交互に振る） | 作業中 |
| `IDLE` | 灰 | 静止 | 入力待ち |
| `DONE` | 緑 | 静止 | バックグラウンドが完了（`--all`） |
| `STOPPED` | 灰 | 静止（足も消える） | 停止済み（`--all`） |
| `UNKNOWN` | 赤 | 静止（足が不揃い） | 未知の状態（生の状態文字列を併記） |
...
`BLOCKED` と `BUSY` は前半の 2 フレーム（右手 → 下ろす）が同じなので、
`BUSY` の後半で左手が上がるかどうかと、色・ステータスバッジで見分けます。
