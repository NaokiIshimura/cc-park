...
| `### 手（1 行目の両端）` | 手のポーズの図 |
| `### 足（3 行目）` | 足の図 |
| `### サブエージェント（ミニキャラクター）` | ミニ AA のフレームと体数ごとの並び |
...
| ミニのサイズ | サブエージェント用のミニキャラクターは **1 行 x 2 桁**。1 桁空けて行の幅いっぱいまで横に並べ、入りきらないときだけ折り返す（省略はしない。折り返した行の間は 1 行空ける） |
...
| セッション数 | 既定ウィンドウ（描画領域 360x608）に収まるのは **6 行**。それ以上だとスクロールバーが写り込む |
...
| 組み立て | `magick -delay 20 -loop 0 frame-*.png -dither None -colors 128 +remap -layers Optimize docs/gui.gif`（`delay 20` = 200ms = `ANIMATION_INTERVAL_MS`） |
| 静止画 | `docs/gui.png` も同じ 1 フレーム目から作る: `magick frame-00.png -dither None -colors 256 docs/gui.png` |

`+remap` で全フレームを共通パレットに揃えないと、フレームごとにパレットが変わって
差分圧縮が効かない（284KB → 40KB）。`-dither None` は平坦な背景に出る点状のノイズを防ぐ。
既定ウィンドウのまま撮るので拡大縮小はしない。`-resize` を挟むと AA のブロック文字がぼける。
